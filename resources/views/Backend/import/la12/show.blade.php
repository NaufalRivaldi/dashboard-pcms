...
@section('card-button')
    <a href="{{ route('import.la12.show.pdf', $siswaInaktif->id) }}" class="btn btn-danger  @if($siswaInaktif->status == 0) disabled @endif"><i class="ti-file"></i> Export PDF</a>

    @if($siswaInaktif->status == 0)
        <button class="btn btn-success" data-toggle="modal" data-target="#modalValidation"><i class="ti-check"></i> Accept</button>
    @endif
@endsection

@section('card-content')
    <!-- Start - Detail siswaInaktif -->
    <div class="row">
        <div class="col-sm-3 font-weight-bold">Kode Cabang</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{{ $siswaInaktif->cabang->kode }}</div>
    </div>
    <div class="row mt-2">
        <div class="col-sm-3 font-weight-bold">Nama Cabang</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{{ $siswaInaktif->cabang->nama }}</div>
    </div>
    <div class="row mt-2">
        <div class="col-sm-3 font-weight-bold">Bulan</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{{ setMonth($siswaInaktif->bulan) }}</div>
    </div>
    <div class="row mt-2">
        <div class="col-sm-3 font-weight-bold">Tahun</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{{ $siswaInaktif->tahun }}</div>
    </div>
    <div class="row mt-2">
        <div class="col-sm-3 font-weight-bold">Status</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{!! statusValidate($siswaInaktif->status) !!}</div>
    </div>
    <div class="row mt-2">
        <div class="col-sm-3 font-weight-bold">Jumlah Total Siswa Inaktif</div>
        <div class="col-sm-1 text-right">:</div>
        <div class="col-sm-8">{{ $siswaInaktif->jumlah }} Orang</div>
    </div>
    <!-- End - Detail siswaInaktif -->
@endsection
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalValidationLabel">Verifikasi Data siswa Inaktif</h5>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            ...
            accept: function(){
                // ----------------------------------------------------------------
                let url = "{{ route('import.la12.show.accept', ':id') }}";
                url = url.replace(':id', "{{ $siswaInaktif->id }}");
