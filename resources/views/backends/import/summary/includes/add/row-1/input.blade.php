...
<div class="form-group row">
    ...
    <div class="col-sm-10 input-form">
        <input type="text" name="bulan_tahun" class="form-control col-sm-6 @if($errors->has('bulan_tahun')) is-invalid @endif date-picker-month" value="{{ $summary->id ? $summary->bulan_tahun : old('bulan_tahun') }}" required>
    ...
    <div class="col-sm-10 input-form">
        <select name="cabang_id" class="form-control select2 @if($errors->has('cabang_id')) is-invalid @endif" required>
            ...
            @foreach($cabangs as $id => $value)
                <option value="{{ $id }}" @if($id == $summary->cabang_id) selected @endif>{{ $value }}</option>
            @endforeach
        </select>
        <!-- Start - Error handling -->
